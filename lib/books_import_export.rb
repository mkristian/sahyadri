...
class BooksImportExport
    ...
    CSV.open(file, 'r') do |row|
      if first
        ...
        if date == "NA" || date.nil? || date.size == 1 || date == "Hindi  play" || date == "Search Dover"
          book.created_at = Date.new(0)
        elsif date =~ /\d+\/\d+\/\d\d\d\d/
          book.created_at = Date.strptime(date, "%d/%m/%Y")
          #puts date + " #{book.created_on}"
        elsif date =~ /\d+\/\d+\/\d\d/
          date.sub!(/\/009\//, '/09/')
          book.created_at = begin
          ...
          book.created_at = Date.strptime("1.07.1996", "%d.%m.%Y") 
        elsif date == "28/06"
          book.created_at = Date.strptime("28.06.1997", "%d.%m.%Y")          
        elsif date =~ /\d+.\d+.\d\d\d\d/
          book.created_at = Date.strptime(date, "%d.%m.%Y")          
          ...
          date.sub!(/Sep[.]/,'Sep')
          date.sub!(/sept/,'Sep')
          book.created_at = Date.strptime(date, "%d %b %y")
        elsif date =~ /\d+\w+[.]? \d\d/ 
          book.created_at = Date.strptime(date, "%d%b %y")
        else
          p row
  ...
  def self.export(file)
    CSV.open(file, 'w') do |writer|
      do_export(Book.all, writer)
    end
  end
  
  def self.do_export(books, writer)
